...
## 严谨准则（老板要求，根规则，与全局 ~/.claude/CLAUDE.md 同源）

1. 事实性断言（行业/文献/数据）→ 先调研，无来源不落笔
2. 评价老板观点 → 客观，同意或反对都要证据，只附和 = 违规
3. 结论带依据链（证据 → 推理 → 结论），注明"证据 or 推测"
4. 本项目的文档（docs/*.md、判断日志、复盘）→ 强制含"依据/来源"字段
