...
"""因子 top30% 多头收益时间序列（周度，A股 long-only 口径）
与多空版的区别：只算 top30% 等权收益，不减 bottom30%（A股不能做空bottom）。
用于 long-only TSMOM 筛因子选股。
"""
...
OUT = os.path.expanduser('~/v5/branches/factor_momentum/factor_returns_top.json')
HORIZON = 5
TOP_PCT = 0.30
...
def main():
    log("=" * 60); log("因子 top30% 多头收益 (周度, long-only)"); log("=" * 60)
    ...
    if all_rets:
        ...
        log(f"全部top30%多头收益: n={len(arr)}, mean={arr.mean():.4f}, t={arr.mean()/(arr.std()/np.sqrt(len(arr))):.2f}")
    db.close()
