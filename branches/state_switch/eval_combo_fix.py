...
"""修复 combo: 低波+反转组合, pd.notna过滤nan
combo = rank(1-vol) + rank(1-cum_ret) 选高分Top10, T+10。
"""
...
TOP = 10
HORIZON = 10
OUT = os.path.expanduser('~/v5/branches/state_switch/eval_combo_fix_result.json')
...
def main():
    log("=" * 60)
    log("combo修复: 低波+反转组合, pd.notna过滤")
    log("=" * 60)
    ...
    for di, date in enumerate(dates):
        ...
        for code in codes:
            ...
            if v is None or cr is None:
                continue
            rows.append({'vol': v, 'cr': cr, 'code': code, 'ret': r})
        if len(rows) < TOP * 2:
            continue
        df = pd.DataFrame(rows)
        df['r_vol'] = 1 - df['vol'].rank(pct=True)  # vol低=高分
        df['r_rev'] = 1 - df['cr'].rank(pct=True)    # 跌多=高分
        df['combo'] = df['r_vol'] + df['r_rev']
        sel = df.sort_values('combo', ascending=False).head(TOP)
        for _, row in sel.iterrows():
            r = row['ret']
            if pd.notna(r):  # 修复: pd.notna 捕获 nan
                all_rets.append(r)
    ...
    log("(对比: 低波Top10 T+20 夏普2.58, 反转 夏普1.33)")
