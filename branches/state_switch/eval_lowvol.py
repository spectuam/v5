...
"""验证低波策略: 20日波动最低Top10, T+10, 2024-2026(排除7月异常)
全段+分年, 夏普/盈亏比/胜率/max_dd/均收。
"""
...
TEST_END = '2026-06-30'  # 排除7月(T+10超范围)
TOP = 10
HORIZON = 10
OUT = os.path.expanduser('~/v5/branches/state_switch/eval_lowvol_result.json')
...
def main():
    log("=" * 60)
    log(f"低波策略验证: vol_20 最低 Top{TOP}, T+{HORIZON}, {TEST_START}~{TEST_END}")
    log("=" * 60)
    ...
    for di, date in enumerate(dates):
        ...
        if len(scored) < TOP:
            continue
        scored.sort(key=lambda x: x[0])  # 波动最低在前
        for i in range(TOP):
            r = scored[i][1]
            all_rets.append(r)
    ...
    log("(对比: 反转全段夏普1.33 盈亏比1.40; 目标夏普>1 盈亏比>2)")
